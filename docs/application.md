...
2. **打轴按键处理（核心逻辑 - 支持多演唱者）**
   ...
   **节奏点导航逻辑**：
   ...
   - **跨演唱者**：用户可手动切换演唱者，继续为该演唱者打轴

5. **切换演唱者**
   - 用户从演唱者列表选择新的演唱者
   - TimingService 更新当前 singer_id
   - 加载该演唱者的当前打轴位置（行/字符/节奏点）
   - UI 切换高亮显示的轨道

6. **多演唱者同时预览**
   - 显示模式切换：单演唱者 / 全部演唱者
   - 全部模式下：同时渲染所有演唱者的歌词
   - 根据播放位置，为每个演唱者独立计算当前高亮字符
   - 使用不同颜色区分不同演唱者
...
3. **切换当前演唱者**
   - 设置 TimingService 的当前 singer_id
   - UI 高亮显示当前演唱者的歌词轨道
   - 打轴操作仅作用于当前演唱者
...
**回调接口**：
- `on_singer_added(singer)`：新演唱者添加
- `on_singer_removed(singer_id)`：演唱者删除
- `on_singer_changed(singer_id)`：当前演唱者切换
- `on_singer_updated(singer)`：演唱者属性修改
