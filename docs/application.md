...
- **职责**：
    ...
    - **打轴计时逻辑**：统一入口 `on_key_changed(timestamp_ms, key_type)`，按键按下/抬起均推送给当前选中 cp，由 cp 角色决定是否写入：普通节奏点仅响应 `pressed`，句尾末尾 cp（`is_sentence_end_tail_cp`）仅响应 `released`（用户必须长按到句子结束）。每次写入后单次推进到下一个 cp。Offset 校准基于按键按下。
