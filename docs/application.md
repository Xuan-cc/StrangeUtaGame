...
- **职责**：
    ...
    - **句尾判定**：提供 `check_space_as_line_end` 开关（默认 ON），当字符后面紧跟空格时，或者作为 Sentence 的最后一个字符时，将其 `is_sentence_end` 标记设为 True。这会使该字符额外增加一个节奏点。注意「句尾」（sentence end）与「行尾」（line end）不同，一行内可以有多个句尾，但只有一个行尾。
