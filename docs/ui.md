...
| `frontend/editor/timing/` | 从 `timing_interface` 拆出的子组件：`TransportBar` / `EditorToolBar` / `KaraokePreview` / `TimelineWidget` / `ModifyCharacterDialog` / `InsertGuideSymbolDialog` / `CharEditDialog` / `_SentenceSnapshotCommand` |
| `frontend/editor/edit_interface.py` | `EditInterface` 行表格编辑 |
| `frontend/editor/ruby_interface.py` | `RubyInterface` 全文本/注音编辑 |
| `frontend/editor/bulk_change_dialog.py` | 批量字符替换对话框（通过 `_SentenceSnapshotCommand` 支持撤销） |
