...
| D | `@Offset` 单位 = 毫秒 (ms) | 导入时映射到 `tags["offset"]` 标准栏位 | ✓ 已合规 | 推荐 | 与 `@HeadOffset` 同为标准栏位（int，0 = 导出时不写该标签）；转换失败回退 custom 保留原文 |
...
- 映射规则：
  ...
  - `Offset` → `tags["offset"] (int)`，`HeadOffset` → `tags["head_offset"] (int)`；转换失败 fallback 到 custom 保留原始行
