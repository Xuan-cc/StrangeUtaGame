...
class TestLyricLine:
    ...
    def test_creation_with_defaults(self):
        """测试使用默认值创建 LyricLine"""
        line = LyricLine(singer_id="singer_1", text="测试歌词")
    ...
    def test_creation_with_custom_chars(self):
        """测试使用自定义 chars 创建"""
        line = LyricLine(singer_id="singer_1", text="测试", chars=["测", "试"])

        assert line.chars == ["测", "试"]

    def test_default_checkpoints_created(self):
        """测试默认节奏点配置自动创建"""
        line = LyricLine(singer_id="singer_1", text="ABC")
        ...
        for i, config in enumerate(line.checkpoints):
            ...
            if config.is_line_end:
                assert config.check_count == 2  # 句尾有两个 checkpoint（按下+抬起）
            else:
    ...
    def test_add_timetag(self):
        ...
        tag = TimeTag(timestamp_ms=1000, singer_id="s1", char_idx=0, checkpoint_idx=0)
    ...
    def test_is_fully_timed(self):
        ...
        # 添加所有需要的时间标签（句尾字符需要 2 个 checkpoint）
        for i in range(3):
            tag = TimeTag(
        ...
        # 还没完成：句尾字符 'C' (char_idx=2) 需要 checkpoint_idx=1
        assert line.is_fully_timed() is False

        # 添加句尾第二个 checkpoint
        tag_end = TimeTag(
            timestamp_ms=2500, singer_id="s1", char_idx=2, checkpoint_idx=1
        )
        line.add_timetag(tag_end)
    ...
    def test_get_timing_progress(self):
        ...
        assert completed == 0
        assert total == 4  # A(1) + B(1) + C(2, 句尾) = 4 个节奏点
