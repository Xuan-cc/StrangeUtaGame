...
class TestProject:
    ...
    def test_creation_with_defaults(self):
        ...
        assert default_singer.backend_number == 1  # 后台编号从1开始
    ...
    def test_get_timing_statistics(self):
        ...
        line1.add_timetag(
            TimeTag(
                timestamp_ms=2000, singer_id=singer.id, char_idx=1, checkpoint_idx=0
            )
        )
        # 'B' 是句尾，check_count=2，还需要 checkpoint_idx=1 才算完成
        line1.add_timetag(
            TimeTag(
                timestamp_ms=2500, singer_id=singer.id, char_idx=1, checkpoint_idx=1
            )
        )
        ...
        assert stats["total_timetags"] == 3
        assert stats["completed_lines"] == 1
        # A(1) + B(2) = 3 per line, 2 lines = 6 total checkpoints; 3 timetags completed
        assert stats["timing_progress"] == "3/6"
