...
- **连词编辑**：F3 toggle linked_to_next 标记，控制字符是否与下一字符相连，每个字符独立标记
...
- **演唱者划词选择**：打轴预览中拖拽选中文字后右键菜单设置演唱者，自动插入占位 timetag，checkpoint 颜色跟随演唱者
...
- 可添加/删除演唱者
- 设置演唱者颜色和名称
- 划词选择功能：在打轴预览中拖拽选中文字 → 右键 → 选择演唱者
- 支持同一行内切换演唱者（per-char 级别），自动插入占位 timetag 携带 singer_id
