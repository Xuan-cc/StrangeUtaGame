...
- **日语注音**：自动为所有字符类型添加注音（汉字、平假名、片假名、英文、数字、符号、空格），支持手动编辑、按字符类型批量删除
...
- **Offset 校准（可视化弹窗）**：独立校准窗口，5 个滑块按 BPM 滑动至红色判定线并播放节拍器音，空格键敲击后自动计算偏移量（区间二等分法），实时显示最近/平均偏移，支持校准中调节 BPM
...
- 弹出独立校准窗口：深色画布上 5 个白色滑块按 BPM 匀速从左到右滑动
- 滑块接近中央红色判定线时自动放大，到达判定线时播放节拍器点击音
