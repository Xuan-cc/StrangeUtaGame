...
- **Offset 校准（可视化弹窗）**：独立校准窗口，2 个滑块按 BPM 滑动至红色判定线，每个滑块穿过判定线对应一拍并播放节拍器音（非阻塞播放），以滑块穿过判定线（视觉正中央）为完美判定点计算偏移量（正=偏早，负=偏晚），实时显示最近/平均偏移，支持校准中调节 BPM
...
- **保存项目**：在任意界面按 Ctrl+S 保存项目（.sug 格式）。若已有保存路径则直接保存，首次保存弹出另存为对话框
