...
- **打轴预览指引**：上一个 / 正在打 / 下一个字的引导着色（不透明度与开关可自定义）+ 导唱符待办标记（`needs_guide`）
...
- **候补字典**：批量变更 / 修改字符中可「查询候补字典」，从本地 + 网络源取多个候选读音
...
| `[` / `]` | 增加 / 减少节奏点 | 同左 |
| `F2` | 在当前字上方打开迷你注音框，可切换是否连接下一字 | 同左 |
| `F3` | 连词/取消连词（可自定义快捷键） | 连词/取消连词 |
| `P` | 打到停顿点（剩余字符全部打戳） | — |
| `.` (长按) | 切换停顿点标记（is_sentence_end） | — |
| `.` (短按) | — | 切换停顿点标记 |
