...
class Project:
    ...
    def remove_singer(self, singer_id: str, 
                      transfer_to: Optional[str] = None) -> None:
        """删除演唱者
    ...
    def add_line(self, line: LyricLine, 
                 after_line_id: Optional[str] = None) -> None:
        """添加歌词行
        ...
        if not self.get_singer(line.singer_id):
            raise ValidationError(
                f"演唱者 {line.singer_id} 不存在，无法添加歌词行"
            )
    ...
    def get_timing_statistics(self) -> Dict[str, Any]:
        ...
        total_checkpoints = sum(
            sum(c.check_count for c in line.checkpoints)
            for line in self.lines
        )
    ...
    def validate(self) -> List[str]:
        ...
        for line in self.lines:
            if line.singer_id not in singer_ids:
                errors.append(
                    f"歌词行 {line.id} 的 singer_id {line.singer_id} 无效"
                )
